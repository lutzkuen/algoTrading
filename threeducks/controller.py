...
class controller(object):
    def __init__(self, confname):
        ...
        self.settings = {}
        self.ducks = [{'name': 'duck1', 'granularity': 'H4', 'numCandles': 60}, {'name': 'duck2', 'granularity': 'H1', 'numCandles': 60}, {'name': 'duck3', 'granularity': 'M5', 'numCandles': 60}]
        ...
        self.settings['takeProfit'] = int(config.get('ducks', 'takeProfit'))
    ...
    def getSMA(self, ins, granularity, numCandles):
        request = Request(
            'GET',
        ...
        sma = np.mean([np.float(candle.get('mid').get('c')) for candle in candles.get('candles')]) # sma over all returned candles
        return sma
    ...
    def checkIns(self, ins):
        ...
        # get the ducks
        smas = [self.getSMA(ins, duck.get('granularity'), duck.get('numCandles')) for duck in self.ducks]
        ...
        if len([ trade for trade in self.trades if trade.instrument == ins and trade.currentUnits*direction > 0]) > 0: 
            print('Skipping ' + ins)
            return
